...
func (r *fakeHTTPContentRouter) numProvideCalls() int {
	r.m.Lock()
	defer r.m.Unlock()
	return r.provideCalls
}
func (r *fakeHTTPContentRouter) clear() {
	r.m.Lock()
	defer r.m.Unlock()
	r.provideCalls = 0
	r.findProvidersCalls = 0
}
...
func TestContentRoutingHTTP(t *testing.T) {
	...
	t.Run("adding a new CID does not result in an HTTP provide", func(t *testing.T) {
		...
		assert.Zero(t, cr.numProvideCalls(), "expected no HTTP router calls")
	})
}
